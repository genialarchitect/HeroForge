...
impl WindowsCollector for ServiceCollector {
    async fn collect(&self, object: &OvalObject, context: &CollectionContext) -> Result<Vec<OvalItem>> {
        ...
        if !context.has_credentials() {
            log::warn!("No WinRM credentials configured, skipping service collection");
            return Ok(vec![]);
        }
        ...
        let script = self.build_service_script(service_name);

        // Execute via WinRM
        let output = match context.execute_script(&script).await {
            Ok(out) => out,
            Err(e) => {
                log::warn!("Failed to execute service collection script: {}", e);
                return Ok(vec![]);
            }
        };
        ...
        let json: serde_json::Value = match serde_json::from_str(&output) {
            Ok(j) => j,
            Err(e) => {
                log::warn!("Failed to parse service collection output: {}", e);
                return Ok(vec![]);
            }
        };
